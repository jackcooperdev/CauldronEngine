...
async function checkManifest(fileName, url) {
    return new Promise(async (resolve,reject) => {
        ...
        try {
            ...
            if (isOnline) {
                try {
                    const downloadedFile = await downloadManifest(url,path.join(CAULDRON_PATH,fileName))
...
async function downloadManifest(url, dir) {
    return new Promise(async (resolve, reject) => {
        var config = {
            method: 'get',
            url: url
        ...
        try {
            const file = await axios(config);
            shelljs.mkdir('-p', path.join(dir,'../'));
            fs.writeFileSync(dir,JSON.stringify(file.data));
            resolve(file.data);
        } catch (err) {
            reject(err.message)
        };
...
async function getManifests(v, l, lv) {
    return new Promise(async (resolve,reject) => {
        ...
        try {
            ...
            const getMain = await checkManifest('cauldron_version_manifest.json','https://launchermeta.mojang.com/mc/game/version_manifest.json');
