...
import { validate } from "../../tools/fileTools.js";
import { cauldronLogger } from "../../tools/logger.js";
import { convertNameToPath } from "./utils.js";
    ...
    try {
      ...
      const installer = new StreamZip.async({
        file: path.join(
          CAULDRON_PATH,
          "forge-installers",
          `forge-${version}-${loaderVersion}-installer.jar`,
        ),
      });
        ...
        if (profileFile.processors.length === 0) {
          ...
          fs.writeFileSync(
            path.join(
              ...
              `forge-${version}-${loaderVersion}.json`,
            ),
            ...
            if (fIdx === "MCP_VERSION") {
              ...
              if (
                ...
                params[fIdx] = path.join(
                  CAULDRON_PATH,
                  "libraries",
                  splitDir[0].replace(/\./g, "/"),
                  splitDir[1],
                  splitDir[2],
                  `${splitDir[1]}-${splitDir[2]}-${splitDir[3]}`.replace(
                    "@",
                    ".",
                  ),
                );
                ...
                params[fIdx] = path.join(
                  ...
                  splitDir[1],
                  splitDir[2],
                  `${splitDir[1]}-${splitDir[2]}-${splitDir[3]}.jar`,
                );
              }
            } else {
          ...
          // Additional Params
          params["MAPPING_PATH"] = path.join(
            CAULDRON_PATH,
            "libraries",
            "de/oceanlabs/mcp/mcp_config",
            `${version}-${MCP_VERSION}`,
            `mcp_config-${version}-${MCP_VERSION}.zip`,
          );
          params["MINECRAFT_JAR"] = path.join(
            CAULDRON_PATH,
            "versions",
            `forge-${version}-${loaderVersion}`,
            `forge-${version}-${loaderVersion}.jar`,
          );
          params["SIDE"] = "client";
