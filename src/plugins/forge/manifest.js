...
async function getManifest(fVersion, version, versionCache) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const installer = new StreamZip.async({
                file: path.join(CAULDRON_PATH, "forge-installers", installObj.fileName),
            });
            await installer.entries();
            const profileFileBuffer = await installer.entryData(
            ...
            if (!profileFile.json) {
                ...
                manifestData = await handleLegacyFormat(
                    fVersion,
                    version,
                    versionCache,
                    profileFile,
                ...
                manifestData = await handleRegFormat(
                    fVersion,
                    version,
                    versionCache,
                    profileFile,
            ...
            shelljs.mkdir(
                "-p",
                path.join(CAULDRON_PATH, "versions", `forge-${version}-${fVersion}`),
            );
            fs.writeFileSync(
                path.join(
                    CAULDRON_PATH,
                    "versions",
                    `forge-${version}-${fVersion}`,
                    `forge-${version}-${fVersion}.json`,
                ),
            ...
            //Merge Libraries
            manifestData.libraries = [
                ...manifestData.libraries,
                ...versionCache.libraries,
            ];
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            for (let idx in libraries) {
                ...
                versionCache.libraries.find((o, i) => {
                    if (!handledLibraries.includes(convertedName)) {
                        ...
                        splitOName.pop();
                        if (
                            splitOName.join(":") === convertedName &&
                            !handledLibraries.includes(convertedName)
                        ) {
            ...
            if (!versionFile.minecraftArguments) {
                manifestData.arguments.game = [
                    ...versionFile.arguments.game,
                    ...versionCache.arguments.game,
                ];
                if (versionFile.arguments.jvm) {
                    manifestData.arguments.jvm = [
                        ...versionCache.arguments.jvm,
                        ...versionFile.arguments.jvm,
                    ];
                } else {
            ...
            if (mainForge.downloads.artifact.url !== "") {
                ...
                for (const entry of Object.values(entries)) {
                    if (
                        entry.name.includes(forgePath.split("\\").join("/")) &&
                        !entry.isDirectory
                    ) {
                ...
                for (let fIdx in filesInMaven) {
                    ...
                    fs.writeFileSync(
                        path.join(CAULDRON_PATH, "libraries", removeMaven),
                        mFileBuffer,
                    );
                }
            }
