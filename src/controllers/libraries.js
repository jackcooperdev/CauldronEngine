...
async function getLibraries(libList, versionData, maniID, customName) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (
                fs.existsSync(path.join(CAULDRON_PATH, "versions", maniID, "natives"))
            ) {
                nativeLock = true;
            }
            ...
            for (let idx in libList) {
                ...
                if (libAllowed) {
                    ...
                    if (libList[idx].downloads.artifact) {
                        shell.mkdir(
                            ...
                            path.join(
                                CAULDRON_PATH,
                                "libraries",
                                libList[idx].downloads.artifact.path,
                                "../",
                            ),
                        );
                        let obj = {
                            ...
                            fileName:
                                libList[idx].downloads.artifact.path.split("/")[
                                libList[idx].downloads.artifact.path.split("/").length - 1
                                    ],
                        };
                        dQueue.push(obj);
                    ...
                    if (
                        libList[idx].downloads.classifiers &&
                        (await checkInternet()) &&
                        !nativeLock
                    ) {
                        let natives =
                            libList[idx].downloads.classifiers[
                                libList[idx].natives[actualOS]
                                ];
                        if (!natives) {
                            if (
                                libList[idx].natives &&
                                libList[idx].natives[actualOS] &&
                                libList[idx].natives[actualOS].includes("arch")
                            ) {
                        ...
                        if (natives) {
                            let obj = {
                                ...
                                fileName:
                                    natives.path.split("/")[natives.path.split("/").length - 1],
                            };
                            ...
                            for (const entry of Object.values(entries)) {
                                if (
                                    ...
                                    await zip.extract(
                                        entry.name,
                                        path.join(CAULDRON_PATH, "versions", maniID, "natives"),
                                    );
                                }
                            }
                            await zip.close();
            ...
            if ((await checkInternet()) && !currentLibraryFile[checkName]) {
                ...
                fs.writeFileSync(
                    path.join(CAULDRON_PATH, "libs_installed.json"),
